...
import { updateGameState } from "./dbComms.js";

export default function BigBoard({ robot, appStatus, playerIsX, gameId, networkChange }) {
  let initalPosition = [];
  for (let i = 0; i < 9; i++) {
  ...
  function updateGame(board, position, x = xNext) {
    if (appStatus !== "localGame"  && xNext !== playerIsX) return;
    ...
    if(appStatus === "networkGame") {
  
      updateGameState(nextPosition, position, gameId);
    }
  ...
  if(appStatus === "networkGame" &&
   ...
   networkChange.operationType === "update" && 
   networkChange.fullDocument.state !== gamePosition){
    setGamePosition(networkChange.fullDocument.state);
    setLastMove(networkChange.fullDocument.lastMove);
   }
